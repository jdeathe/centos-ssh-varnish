...
OPT_SERVICE_NAME_FULL=${SERVICE_NAME_FULL:-varnish.pool-1.1.1@8000.service}
OPT_SERVICE_NAME_SHORT=$(cut -d '@' -f1 <<< "${OPT_SERVICE_NAME_FULL}")

# Add required configuration directories
mkdir -p /etc/services-config/${OPT_SERVICE_NAME_SHORT}/{varnish,supervisor}
...
if [[ ! -n $(find /etc/services-config/${OPT_SERVICE_NAME_SHORT}/varnish -maxdepth 1 -type f) ]]; then
	cp -R etc/services-config/varnish /etc/services-config/${OPT_SERVICE_NAME_SHORT}/
fi

# Stop remove any existing application and configuration volume containers.
